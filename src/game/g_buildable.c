...
void G_BuildableThink( gentity_t *ent, int msec )
{
  ...
  if( !ent->spawned && ent->health > 0 && !level.pausedTime )
  {
    if( ent->buildTime + buildTime < level.time )
    {
      ...
      {
        G_TeamCommand( TEAM_ALIENS, "cp \"Cocoon now available!\"" );
      }
    }
  }
  ...
  if( ent->time1000 >= 1000 )
  {
    ...
    else if( ent->health > 0 && ent->health < maxHealth )
    {
      ...
      {
        ent->health += MAX(HUMAN_BREGEN_MIN,(int)((float)HUMAN_BREGEN_MODIFIER*g_humanDefenceComputerRate.integer * g_humanDefenceComputerLimit.integer));
      }
    }
